...
<div class="flex-grow">
    ...
    <form id="data" action="{{ route('blog.control-panel.articles.store') }}" method="POST">
        ...
        <label class="flex items-center justify-between mb-4">
            ...
            <input class="w-4/5 bg-white border border-solid border-gray-600 rounded py-1 px-2" name="title" type="text" placeholder="{{ __('blog.article-form-placeholder-title') }}" minlength="3" maxlength="200" required value="{{ old('title') }}">
        </label>
        <label class="flex items-start justify-between mb-4">
            <span class="font-bold mr-8 select-none">{{ __('blog.article-form-content') }}</span>
            <textarea class="w-4/5 h-40 resize-none bg-white border border-solid border-gray-600 rounded py-1 px-2" name="content" placeholder="{{ __('blog.article-form-placeholder-content') }}" required minlength="5" maxlength="10000">{{ old('content') }}</textarea>
        </label>
        <label class="flex items-start justify-between mb-4">
            <span class="font-bold mr-8 select-none">{{ __('blog.article-form-fragment') }}</span>
            <textarea class="w-4/5 h-40 resize-none bg-white border border-solid border-gray-600 rounded py-1 px-2" name="fragment" placeholder="{{ __('blog.article-form-placeholder-fragment') }}" maxlength="500">{{ old('fragment') }}</textarea>
        </label>
        <label class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="flex items-start w-4/5">
                <input id="checkbox" class="rounded w-4 h-4" name="is_published" type="checkbox" value="1" @if(old('is_published') === '1') checked @endif>
            </span>
        </div>
    </form>
