...
<div class="flex-grow">
    <div class="pb-6 flex justify-between">
        <div>
            <a href="{{ route('blog.control-panel.trash.categories.index') }}" class="flex items-center justify-center w-32 h-8 text-gray-600 bg-white border border-solid border-gray-600 rounded">{{ __('blog.btn-back') }}</a>
        </div>
        @include('blog.components.control-panel.menu')
    </div>
    <form id="data" action="{{ route('blog.control-panel.trash.categories.update', $category->id) }}" method="POST">
        @method('PATCH')
        @csrf
    ...
    <form id="delete" action="{{ route('blog.control-panel.trash.categories.destroy', $category->id) }}" method="POST">
        @method('DELETE')
        @csrf
    </form>
