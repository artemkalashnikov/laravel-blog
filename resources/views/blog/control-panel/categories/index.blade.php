...
<div class="flex-grow">
    <div class="pb-6 flex justify-between">
        <div>
            <a href="{{ route('blog.control-panel.categories.create') }}" class="flex items-center justify-center w-40 h-8 text-white bg-red-700 border border-solid border-red-700 rounded">{{ __('blog.btn-add-category') }}</a>
        </div>
    ...
    @else
        <table class="table-fixed w-full">
            ...
            @foreach($categoriesPaginator as $category)
                <tr>
                    <td class="px-1 py-2"><a class="text-red-700 underline hover:no-underline hover:text-red-700" href="{{ route('blog.control-panel.categories.edit', $category->id) }}">{{ $category->title }}</a></td>
