...
<div class="flex-grow">
    <div class="pb-6 flex justify-between">
        <div>
            <a href="{{ route('blog.control-panel.trash.articles.index') }}" class="flex items-center justify-center w-32 h-8 text-gray-600 bg-white border border-solid border-gray-600 rounded">{{ __('blog.btn-back') }}</a>
        </div>
        @include('blog.components.control-panel.menu')
    </div>
    <form id="data" action="{{ route('blog.control-panel.trash.articles.update', $current_article->id) }}" method="POST">
        @method('PATCH')
        @csrf
        <label class="flex items-center justify-between mb-4">
            <span class="font-bold mr-8 select-none">{{ __('blog.article-form-title') }}</span>
            <input class="w-4/5 bg-white border border-solid border-gray-600 rounded py-1" name="title" type="text" placeholder="{{ __('blog.article-form-placeholder-title') }}" minlength="3" maxlength="200" required value="@if(empty(old('title'))){{ $current_article->title }}@else{{ old('title') }}@endif">
        </label>
        <label class="flex items-start justify-between mb-4">
            <span class="font-bold mr-8 select-none">{{ __('blog.article-form-content') }}</span>
            <textarea class="w-4/5 h-40 resize-none bg-white border border-solid border-gray-600 rounded py-1" name="content" placeholder="{{ __('blog.article-form-placeholder-content') }}" required minlength="5" maxlength="10000">@if(empty(old('content'))){{ $current_article->content }}@else{{ old('content') }}@endif</textarea>
        </label>
        <label class="flex items-start justify-between mb-4">
            <span class="font-bold mr-8 select-none">{{ __('blog.article-form-fragment') }}</span>
            <textarea class="w-4/5 h-40 resize-none bg-white border border-solid border-gray-600 rounded py-1" name="fragment" placeholder="{{ __('blog.article-form-placeholder-fragment') }}" maxlength="500">@if(empty(old('fragment'))){{ $current_article->fragment }}@else{{ old('fragment') }}@endif</textarea>
        </label>
        <label class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-start justify-between mb-4">
            ...
            <span class="flex items-start w-4/5">
                <input id="checkbox" class="rounded" name="is_published" type="checkbox" value="1" @if(old('is_published') === '1' || (old('is_published') !== '0' && $current_article->is_published === 1)) checked="checked" @endif>
            </span>
        </div>
    </form>
    <form id="delete" action="{{ route('blog.control-panel.trash.articles.destroy', $current_article->id) }}" method="POST">
        @method('DELETE')
        @csrf
    </form>
    <div class="flex justify-end">
        ...
        <button form="data" class="w-40 h-8 text-white bg-red-700 border border-solid border-red-700 rounded" type="submit">{{ __('blog.btn-restore') }}</button>
    </div>
    <p class="pt-4">{{ __('blog.btn-restore-description') }}</p>
</div>
